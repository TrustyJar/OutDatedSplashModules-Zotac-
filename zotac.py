...
def solveCaptcha(sitekey):
    api_key = "cbc83cc83bb9b6187dd71583548dea12"
...
def scrape_main_site():
    ...
    key = soup.find('h-captcha')['data-sitekey']
    result = "null"
    if(key):
        result = solveCaptcha(key)
    if(result!="null"):
        soup.find('#g-recaptcha-response').insert(0,result)
